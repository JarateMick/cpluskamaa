...
typedef Graph<MapNode, int> MapGraphType;
MapGraphType nodes(64);
...
void SetNeighbours(std::vector<int> * newNeighbours, int id, MapGraphType* mapGraph)
{
	mapGraph->nodes[id]->archlist.clear();

	for (int neighboursId : *newNeighbours)
	{
		ASSERT(neighboursId != id);
		mapGraph->AddArc(id, neighboursId, 1);
	}
}
...
void VisualizePath(std::vector<int>* path, game_state* state)
{
	...
	v2 lastPos = state->provinceData.positions[path->at(0)];
	for (int i = 0; i < path->size(); i++)
	{
		int provinceID = path->at(i);
		auto v2 = state->provinceData.positions[i];
...
EXPORT void Loop(EngineCore* core)
{
	...
	if (!core->memory->isInitialized)
	{
		...
		// editor
		gameState->provinceEditor.selectedNeighbours.reserve(10);
	...
	{
		ProvinceEditorData& editor = gameState->provinceEditor;
		if (gameState->provinceEditor.selectedProvinceId != -1)
		{
			SetNeighbours(&editor.selectedNeighbours, editor.selectedProvinceId, &nodes);
			SaveNodes();
		}
...
EXPORT void Draw(EngineCore* core)
{
	...
	{
		auto path = BreadthFirst(0, &nodes, 2);
		path.push_back(2);
