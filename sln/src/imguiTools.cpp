...
EXPORT IMGUIFUNC(Imgui)
{
	...
	ImGui::Checkbox("select target (g)", &selectTarget);    // TODO: Tämä on gameplay kkoooodiaaa
	if (selectTarget)
	{
	...
	if (input->isMouseClicked() && editNodes)
	{
		...
		if (id != -1 && std::find(neighbours->begin(), neighbours->end(), id) == neighbours->end())
		{
			// neighbours.push_back(id);
			gameState->provinceEditor.selectedNeighbours.push_back(id);
	...
	{
		// neighbours.clear();
		gameState->provinceEditor.selectedNeighbours.clear();
		GetConsoleInstance().AddLog("Cleared list");
	}

	ImGui::Text("Neighbours ");
	for (int i = 0; i < gameState->provinceEditor.selectedNeighbours.size(); i++)
	{
		ImGui::Text("%i", gameState->provinceEditor.selectedNeighbours[i]); // Setti olis parempi ei samoja 2 kpl
		gameState->provinceEditor.selectedProvinceId = gameState->worldmap.editor.inputProvinceId;
	}
